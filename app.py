...
# Ensure a local 'data' directory exists for SQLite fallback
db_dir = os.path.join(app.root_path, "data")
os.makedirs(db_dir, exist_ok=True)
default_sqlite_uri = f"sqlite:///{os.path.join(db_dir, 'expenses.db')}"

# Fetch Database URL from Environment Variable (for Render PostgreSQL)
db_url = os.environ.get("DATABASE_URL", default_sqlite_uri)
...
def index():
    ...
    monthly_spent = (
        db.session.query(func.sum(Expense.total))
        .filter(Expense.date.like(f"{current_month}%"))
        .scalar()
        or 0
    )
...
def dashboard():
    ...
    for category in categories:
        ...
        percentage = (
            (monthly_spent / category.budget * 100)
...
def add_expense():
    ...
    if request.method == "POST":
        ...
        if not category_id:
            ...
            return render_template(
                "add_expense.html", categories=categories, now=datetime.now()
            )
        ...
        try:
            ...
            db.session.commit()
            flash(
                f"Expense added successfully! {item_name} - {total:.2f} KSH",
                "success",
            )
...
def add_category():
    ...
    try:
        new_category = Category(
            name=category_name, budget=float(category_budget)
        )
...
def export_excel():
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        for column in df:
            ...
            writer.sheets["Expenses"].column_dimensions[
                chr(65 + col_idx)
...
def api_budgets():
    ...
    categories = Category.query.all()
    return jsonify([{"name": c.name, "budget": c.budget} for c in categories])
...
try:
    ...
    print(f"Database initialization deferred or already run: {e}")
